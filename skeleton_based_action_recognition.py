...
class SkeletonActionRecognitionNode:
    ...
    def callback(self, data):
        ...
        # Run pose estimation
        poses, cordinateY = self.pose_estimator.infer(image)
        if len(poses) > 2:
        ...
        image = cv2.rectangle(image, (0, 850), (820, 910), (0, 0, 0), -1)
        image = cv2.rectangle(image, (0, 850), (300, 950), (0, 0, 0), -1)

        cv2.putText(image,  str(self.toc), (10, 890), font, fontScale, (255, 255, 255), thickness, cv2.LINE_AA)
        cv2.putText(image, conf, (10, 940), font, fontScale, (255, 255, 255), thickness, cv2.LINE_AA)

        
        #print(category)
        #print("secondS:", self.toc)
        self.hadi[self.counter][0] = category.confidence
        self.hadi[self.counter][1] = self.toc 
        self.hadi[self.counter][2] = category.data

        df = pd.DataFrame(self.hadi).T   

        df.to_excel(excel_writer = "/home/mako/Desktop/ssss.xlsx") 
